...
async function db(){
  const statements = [
    ...
    sql`CREATE TABLE IF NOT EXISTS builds(id UUID PRIMARY KEY DEFAULT gen_random_uuid(), project_id TEXT NOT NULL, status TEXT DEFAULT 'queued', platform TEXT DEFAULT 'android', logs TEXT DEFAULT '', created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW())`
  ];
...
app.get("/api/projects/:id/code",async c=>{const u=await currentUser(c);if(!u)return c.json({error:"Not logged in"},401);const id=c.req.param("id");const rows=await sql`SELECT * FROM projects WHERE id=${id} AND user_id=${u.id}`;if(!rows.length)return c.json({error:"Project not found"},404);const p=rows[0];return c.json({projectId:p.id,language:"Flutter/Dart",entry:"lib/main.dart",code:flutterCode(p.spec||{})})});
app.get("/api/projects/:id",async c=>{const u=await currentUser(c);if(!u)return c.json({error:"Not logged in"},401);const id=c.req.param("id");const rows=await sql`SELECT p.*, (SELECT count(*) FROM builds b WHERE b.project_id=p.id) AS builds FROM projects p WHERE p.id=${id} AND p.user_id=${u.id}`;if(!rows.length)return c.json({error:"Project not found"},404);return c.json({project:rows[0]})});
...
function flutterCode(spec:any){
  const title=String(spec?.appName||"My App").replace(/[^A-Za-z0-9 ]/g,"").trim()||"My App";
  const screens=Array.isArray(spec?.screens)?spec.screens:["Home"];
  const body=screens.map((s:string)=>"  Widget "+s.replace(/\\W/g,"")+"Screen() => Scaffold(appBar: AppBar(title: Text('"+s.replace(/'/g,"\\'")+"')), body: const Center(child: Text('"+s.replace(/'/g,"\\'")+" screen')));").join("\\n");
  return "import 'package:flutter/material.dart';\\n\\nvoid main() => runApp(const ParasApp());\\n\\nclass ParasApp extends StatelessWidget {\\n  const ParasApp({super.key});\\n  @override Widget build(BuildContext context) => MaterialApp(debugShowCheckedModeBanner:false,title:'"+title.replace(/'/g,"\\'")+"',home: const HomeScreen());\\n}\\n\\nclass HomeScreen extends StatelessWidget {\\n  const HomeScreen({super.key});\\n  @override Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('"+title.replace(/'/g,"\\'")+"')), body: ListView(children:[ "+screens.map((s:string)=>"ListTile(title:Text('"+s.replace(/'/g,"\\'")+"'),trailing:const Icon(Icons.arrow_forward),onTap:(){})").join(",")+" ]));\\n}\\n\\n// Generated by Paras AI\\n// Screens: "+screens.join(", ")+"\\n";
}
