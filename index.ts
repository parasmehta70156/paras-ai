...
function specFromIdea(idea:string){
  ...
  return {
    ...
    features:["Authentication","Responsive UI","Project data","Settings"],
    dataModels:["User","Project","Build"],
    navigation:"Bottom navigation with protected screens",
    androidPackage:"ai.paras.generated"
  };
}

const page = `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Paras AI</title>
<style>
*{box-sizing:border-box}body{margin:0;background:#07080c;color:#f7f7fb;font-family:Inter,system-ui,sans-serif}button,input,textarea{font:inherit}
.wrap{max-width:1180px;margin:auto;padding:24px}.top{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #20232d;padding-bottom:18px}.logo{font-size:25px;font-weight:800}.muted{color:#969baa}.grid{display:grid;grid-template-columns:260px 1fr;gap:20px;margin-top:20px}.side,.card{background:#10121a;border:1px solid #242734;border-radius:16px}.side{padding:16px;height:max-content}.nav{padding:12px;border-radius:10px;margin:4px 0;color:#b8bdcb;cursor:pointer}.nav.active,.nav:hover{background:#1b1e29;color:#fff}.main{min-width:0}.hero{padding:28px;border-radius:18px;background:linear-gradient(135deg,#151827,#0c0e15);border:1px solid #282c3a}.hero h1{font-size:38px;margin:0 0 10px}.input{width:100%;background:#0a0c12;border:1px solid #2b2f3b;color:#fff;border-radius:12px;padding:14px;margin-top:10px;outline:none}.btn{border:0;border-radius:11px;padding:12px 18px;background:#fff;color:#090a0e;font-weight:700;cursor:pointer}.btn.secondary{background:#1d2130;color:#fff;border:1px solid #303544}.row{display:flex;gap:10px;align-items:center}.cards{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:16px}.card{padding:18px}.screen{padding:10px 12px;border:1px solid #292d39;border-radius:10px;margin:8px 0;background:#0c0e14}.hide{display:none}.auth{max-width:460px;margin:80px auto}.error{color:#ff7d8a;margin-top:10px}.ok{color:#72e0a0;margin-top:10px}@media(max-width:800px){.grid{grid-template-columns:1fr}.side{display:none}.cards{grid-template-columns:1fr}.hero h1{font-size:30px}}
</style></head><body><div class="wrap">
<div id="auth" class="auth card"><div class="logo">✦ Paras AI</div><p class="muted">Build apps from ideas with AI.</p><input id="name" class="input" placeholder="Name"><input id="email" class="input" placeholder="Email"><input id="pass" class="input" type="password" placeholder="Password"><div class="row" style="margin-top:12px"><button class="btn" onclick="signup()">Create account</button><button class="btn secondary" onclick="login()">Login</button></div><div id="msg"></div></div>
<div id="app" class="hide"><div class="top"><div class="logo">✦ Paras AI</div><div class="row"><span id="who" class="muted"></span><button class="btn secondary" onclick="logout()">Logout</button></div></div>
<div class="grid"><aside class="side"><div class="nav active">AI Builder</div><div class="nav">Projects</div><div class="nav">Templates</div><div class="nav">Builds</div><div class="nav">Settings</div></aside>
<main class="main"><section class="hero"><div class="muted">AI APP BUILDER</div><h1>Turn your idea into an app.</h1><p class="muted">Describe your Android app. Paras AI creates the project specification, screens, features and build job.</p><textarea id="idea" class="input" rows="5" placeholder="Example: Create a food delivery app with login, restaurants, cart, orders and live tracking."></textarea><div class="row" style="margin-top:12px"><button class="btn" onclick="generate()">Generate App</button><button class="btn secondary" onclick="newProject()">New Project</button></div><div id="result"></div></section>
<div class="cards"><div class="card"><b>Projects</b><h2 id="pc">0</h2><span class="muted">Saved in Postgres</span></div><div class="card"><b>Generated screens</b><h2 id="sc">0</h2><span class="muted">From your idea</span></div><div class="card"><b>Builds</b><h2 id="bc">0</h2><span class="muted">Android jobs</span></div></div></main></div></div>
</div>
<script>
let current=null;
...
async function load(){const d=await api('/api/projects');pc.textContent=d.projects.length;bc.textContent=d.projects.reduce((a,p)=>a+(p.builds||0),0);sc.textContent=d.projects.reduce((a,p)=>a+((p.spec?.screens||[]).length),0)}
async function generate(){try{const ideaEl=document.getElementById('idea');if(!ideaEl.value.trim())throw Error('App idea likho');const d=await api('/api/projects',{method:'POST',body:JSON.stringify({idea:ideaEl.value})});result.innerHTML='<div class="card" style="margin-top:16px"><h3>'+d.project.name+'</h3><p class="muted">'+d.project.spec.description+'</p><b>Screens</b>'+d.project.spec.screens.map(s=>'<div class="screen">'+s+'</div>').join('')+'<p><b>Features:</b> '+d.project.spec.features.join(', ')+'</p><button class="btn" onclick="build(\''+d.project.id+'\')">Create Android Build</button><div id="buildmsg"></div></div>';load()}catch(e){result.innerHTML='<div class="error">'+e.message+'</div>'}}
async function build(id){try{const d=await api('/api/projects/'+id+'/builds',{method:'POST'});buildmsg.innerHTML='<div class="ok">Build #'+d.build.id+' queued. Status: '+d.build.status+'</div>';load()}catch(e){buildmsg.innerHTML='<div class="error">'+e.message+'</div>'}}
function newProject(){idea.value=''}
boot()
</script></body></html>`;
